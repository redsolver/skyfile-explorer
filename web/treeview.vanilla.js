...
class TreeView {
    ...
    async _render(id, level, insertAfterEl) {
        ...
        for (const { id, label, icon, state, link } of children) {
            const metadata = { id, label, level, link, icon, state: state || CollapsibleState.None, loading: false };
...
class VanillaTreeView extends TreeView {
    renderNode(node) {
        ...
        if (node.link) {
            ...
            link.classList.add('far', 'fa-up-right-from-square'); // fa-solid
